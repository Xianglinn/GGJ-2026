...
public class ItemTooltipTrigger : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private ISlot slot; // 是否挂载在 Slot 上 (支持 InventorySlot, WorkBenchSlot, BlueprintSlot)
    ...
    private void Awake()
    {
        ...
        // 尝试获取 Slot接口 (用于所有类型的 Slot)
        // 使用 GetComponentInParent 以兼容触发器在子对象的情况 (如 Slot Image)
        slot = GetComponentInParent<ISlot>();
    }
